...
while read -r TICKET; do
	...
	curl 'https://github.com/_graphql' \
		...
		-H 'priority: u=1, i' \
		-H 'referer: https://github.com/SE-legacy/asm-exam-3sem/issues/new?template=Blank+issue' \
		...
		--data-raw '{"query":"'"$QUERY_ID"'","variables":{"fetchParent":false,"input":{"body":"Задача сгенерирована автоматически. Нужно написать билет '"$TICKET"'","parentIssueId":null,"repositoryId":"R_kgDONrnSXw","title":"Билет '$i': '"$TICKET"'"}}}'
	((i++))
	sleep 3
done <"$TICKETS"
