...
- `cmpsw`;
- `cmpsd`;
- `cmps M, M` --- сравнивает байт, слово или двойное слово в зависимости от размера предоставленных явных операндов (который конечно же должен быть одинаковым). После операции изменяет регистры SI и DI. Абсолютно также, как и с `movs M, M`, лучше избегать использования этой формы, но в случае, если надо переопределить селектор DS на какой-нибудь другой селектор, то эта форма позволяет такое сделать.
...
- `scasw`;
- `scasd`;
- `scas M` --- в зависимости от размера операнда `M`, сравнивает значение в регистре со значением в памяти по адресу ES:DI, после чего изменяет DI в соответствии с флагом DF. Нет никакого смысла использовать, так как приемник ES:DI никоим образом заменить нельзя.
...
- `insw`;
- `insd`;
- `ins M, DX` --- в зависимости от размера `M`, считывает байт, слово или двойное слово из порта с номером DX в память по адресу ES:DI. Так же лучше избегать этой формы, потому что никакого преимущества у нее нет (ES:DI заменить нельзя).

И наконец, команды `outs` записывают в порт ввода/вывода с номером DX значение в памяти, находящееся по адресу DS:SI:
- `outsb`;
- `outsw`;
- `outsd`;
- `outs DX, M` --- история повторяется... Команда позволяет заменить селектор DS на какой-то другой.
