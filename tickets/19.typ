...
где `X` соответствует оному из условий для передачи управления. Если условие выполняется, то будет совершен прыжок, иначе выполнение продолжится следующей идущей командой. `<метка>` должна стоять от `jX` не дальше чем на $-128$ или $127$ байт.
...
вычтет из `op1` `op2`. Результат никуда сохранится, но флаги будут изменены в соответствии с результатом. Чтобы произвести проверку на определенное условие, нужно после `cmp` вызвать команду в соответствующей ячейке из таблицы:
...
выполнит побитовое `and` над операндами и установит флаги в нужные значения (более предпочтительна при проверке чисел на равенство).
