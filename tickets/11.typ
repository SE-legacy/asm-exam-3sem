...
```
По умолчанию метка имеет тип `near`. Если управление должно передаться на метку в другом кодовом сегменте, то та метка должна быть объявлена `public M2`, а в сегменте, из которого происходит прыжок, должно быть `extrn M2: far` (см @public)
