...
```
По умолчанию метка имеет тип `near`. Если управление должно передаться на метку в другом кодовом сегменте, то та метка должна быть объявлена ```asm public M2```, а в сегменте, из которого происходит прыжок, должно быть ```asm extrn M2: far``` (см @public)
...
Процедура типа `near` может быть вызвана только из того кодового сегмента, в котором она содержится, а процедура `far` --- из любого кодового сегмента. При обращении к процедуре `near` в стеке сохраняется адрес команды, следующей за `call` (IP). При обращении к процедуре `far` в стеке сохраняется адрес текущего кодового сегмента и адрес следующей за `call` команды (CS:IP).

== Возврат из процедуры

Существуют следующие варианты:
- ```asm ret [N]``` --- возврат из текущей процедуры;
- ```asm retn [N]``` --- возврат из процедуры типа `near`;
- ```asm retf [N]``` --- возврат из процедуры типа `far`.
