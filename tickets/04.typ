...
Команда занимает в памяти от 1 до 15 байт.

#align(center)[
  ```asm
  [<симв. имя>[:]] <код операции> [<операнд>, ...] ; [комментарии]
  ```
]

x86 работает с безадресными, одноадресными, двухадресными и трехадресными командами.

Символическое имя, если оно есть, представляет собой метку, используемую при передаче управления на эту команду. Если после метки стоит ":", то эта она называется внутренней (управление не нее передать можно только из того кодового сегмента, в котором она содержится)
