...
- `rept n`, где `n` ---  константное выражение. В результате `n` копий тела блока повторения записывается на его месте.
  ...
  ; Arr db 0
      ...
      ; db 255
  ```
- `irp p, <v1, v2, ..., vn>`, где `p` ---  имя формального параметра, который является локальным в теле блока повторения. Тело блока повторится `n` раз, последовательно подставляя в тело вместо `p` значения `v1`, `v2`, ..., `vn`.
...
- `irpc p, <s1s2...sn>` --- работает примерно так же, как и `irp`, только последовательно подставляет не строки, а символы `s1`, `s2`, ..., `sn`.
