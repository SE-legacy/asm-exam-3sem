...
Размещать подпрограммы лучше так, чтобы исполнение их кода не происходило без команды `call`. То есть до первой исполняемой команды или после `ret` главной подпрограммы. А если общий размер подпрограмм велик, то их можно разместить в отдельном кодовом сегменте --- тогда используются директивы `public` и `extrn`.
...
+ *Через регистры* --- параметры записываются в регистры, процедура сразу их использует. Самый простой и эффективный способ. Но нельзя передать большое число параметров, так как свободные регистры могут закончиться.
...
+ *Через стек* --- параметры отправляются в стек командами `push`, а подпрограмма извлекает их из стека с помощью регистров SP и BP. Оптимальный способ, используется современными языками программирования.
...
+ *В блоке параметров* --- в сегменте данных создается специальный блок, содержащий все необходимые параметры. Вызываемая подпрограмма получает адрес этого блока любым вышеперечисленным способом или даже из другого блока параметров.
  ...
  Пример такой передачи данных: функция DOS, находящая первый файл, удовлетворяющий ASCIIZ-спецификации (```asm int 21h```, AH = 4Eh). Эта функция возвращает блок DTA, если файл был найден.
