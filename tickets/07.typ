...
+ Размер передаваемых данных определяется типом операндов в команде.
  ...
  Это важно, поскольку команда `mov [SI], 0` вызовет ошибку сборки. В этом случае надо использовать оператор `ptr`. Тогда следующие команды будут допустимыми и эквивалентными:
  ```asm
  mov byte ptr [SI], 0
  ; или
  mov [SI], byte ptr 0
