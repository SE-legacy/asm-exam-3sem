...
+ Байты в памяти хранятся в прямом порядке, а регистрах --- в обратном. Команда `mov` это учитывает:
  ...
  ```
  *Замечание*: некорректно говорить, что в регистрах байты хранятся "в обратном порядке". Здесь, точно так же, как и в памяти, байты хранятся в порядке увеличения их номеров. Просто память обычно изображается как вещь, растущая справа налево, а регистр --- как вещь, имеющая младшие байты справа, а старшие слева.
