...
Для описания полей используются директивы `dX`. Но имена, использующиеся с этими директивами, не локализованы, и поэтому должны быть уникальны для всей программы. Поля не могут иметь тип структуры. Поля могут иметь *значения по умолчанию*, а могут не иметь (`?`).

Пример объявления структуры:

#grid(columns: (40%, auto), align: horizon + center)[
  ```asm
  Date struc
      y dw 2000
      m db ?
  ...
  #table(
    ...
    stroke: 1pt,
    table.cell(rowspan: 3, stroke: none)[`Date`],
    table.cell(stroke: none, colspan: 2)[2 байта],
    table.cell(stroke: none)[1 байт],
    table.cell(stroke: none)[1 байт],
    table.cell(colspan: 2)[`y`], [`m`], [`d`],
    table.cell(stroke: none)[+0000],
    table.cell(stroke: none)[+0001],
    table.cell(stroke: none)[+0002],
    table.cell(stroke: none)[+0003],
  )
]
...
+ Нельзя присваивать начальное значение полям-массивам

  ```asm
  arr dw 1, 2, 3
  ```
  В том числе нельзя использовать `?`. Но в случае `db` можно использовать в качестве начального значения строку.
+ Поле-строка, объявленное любым образом
  ```asm
  s db 10 dup(?)
  t db 21 dup(0)
  v db 's', 't', 'r'
  ...
  может иметь начальное значение, но
