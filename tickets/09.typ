...
- ```asm
  push R ;добавить значение регистра R в стек
  ```
- ```asm
  pop R ;извлечь элемент из стека и положить значение в регистр R
  ```
- `pusha`/`popa` (i186 и новее) --- добавить/извлечь из стека сразу несколько определенных регистров. Значения добавляются в следующем порядке: AX, BX, CX, DX, SP, BP, SI, DI, а извлекаются в обратном.
...
Например, команда `int 21h` вызовет одну из DOS-функций, команда `int 10h` --- функцию видео драйвера, `int 16h` --- функцию драйвера клавиатуры.
