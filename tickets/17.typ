...
- ```asm
  not R/M ; в отличие от других логических операций, не меняет значение флагов
  ```
...
- `rcl`, `rcr` --- выполняют циклические сдвиги, причем исходное значение флага CF попадает в младший разряд, а вышедший за разрядную сетку старший разряд после этого попадает в CF (см. @shift).
