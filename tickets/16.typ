...
$
  X + Y = cases(
    (X + Y ) mod 2^k = X + Y\, & space C F = 0\, & space "если" X + Y < 2^k,
    (X + Y ) mod 2^k = X + Y − 2^k\, & space C F = 1\, & space "если" X + Y gt.eq 2^k
  )
$
...
$ 250 + 10 = 260 = 100000100_2, "и результат:" 00000100_2 = 4, space C F = 1 $

$
  X − Y = cases(
    (X − Y ) mod 2^k = X − Y\, & space C F = 0\, & space "если" X gt.eq Y,
    (X − Y ) mod 2^k = X + 2^k − Y\, & space C F = 1\, & space "если" X < Y
  )
$

Пример (для байтов):

$ 1 − 2 = 1 + 2^8 − 2 = 257 − 2 = 255, space C F = 1 $
